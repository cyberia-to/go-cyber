...
This means mass-linking dilutes influence rather than amplifying it. The system economically incentivizes selective, high-quality linking: each additional cyberlink reduces the weight of all previous ones. Combined with the bandwidth cost (each cyberlink burns volts), there is a double filter — bandwidth limits quantity, rank algorithm limits influence per link.
...
2. Calculation runs asynchronously in a goroutine (does not block consensus).
...
- `correction`: accounts for dangling nodes (particles with no outgoing links) redistributing rank uniformly.
